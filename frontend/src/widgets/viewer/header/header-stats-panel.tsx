...
export const RouteHeaderStatsWrapper = ({ isDesktop }: { isDesktop: boolean }) => {
	const topicMapId = useViewerStore((s) => s.topicMapId);
	const isCourseAdded = useViewerStore((s) => s.isCourseAdded);
	const nodes = useNodeViewerStore((s) => (s.nodes));
	...
	const topicCount = total;
	const nodeCount = isCourseAdded ? nodes.length : 0;
	...
	return isDesktop ? (
		<RouteHeaderStatsPanelDesktop {...commonProps} />
	) : (
		<RouteHeaderStatsPanelMobile {...commonProps} />
	);
};
...
export const RouteHeaderStatsPanelMobile = ({
  ...
  return (
    ...
    >
      <div
        className={cn('flex items-center justify-between gap-2', isExpanded && 'mb-2')}
      >
        <Badge
          variant="secondary"
