...
import { useFindTopicMapByRouteIdQuery } from '@/shared/graphql/generated/output';
...
export default function Page() {
	...
	const { setNodes } = useNodeStore((state) => ({setNodes: state.setNodes}), shallow);
	const { setEdges } = useEdgeStore((state) => ({setEdges: state.setEdges}), shallow);

	useEffect(() => {
		if (!data?.findTopicMapByRouteId || initialized.current) return;
		
		const { nodeData, edgeData } = data.findTopicMapByRouteId;

		const nodes = nodeData ? JSON.parse(nodeData) : [];
		const edges = edgeData ? JSON.parse(edgeData) : [];
